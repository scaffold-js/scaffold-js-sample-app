(function emailDirectiveTests () {
	...
	describe ('email directive', function emailDirective () {
		beforeEach (function beforeEach () {
			...
			this.$compile (angular.element ('<form name="form"><input scaffold-js-sample-app-email name="email" ng-model="email" /></form>')) (this.$rootScope);
	...
	describe ('email change directive', function emailDirective () {
		beforeEach (function beforeEach () {
			...
			this.$httpBackend.whenGET ('account/?email=test').respond (200);
			this.$compile (angular.element ('<form name="form"><input scaffold-js-sample-app-email-change name="email" ng-model="email" /></form>')) (this.$rootScope);
			this.form = this.$rootScope.form;
		});
		...
		it ('should gnore if matches editing', function errorIfExists () {
			this.$compile (angular.element ('<form name="form"><input scaffold-js-sample-app-email-change="origEmail" name="email" ng-model="email" /></form>')) (this.$rootScope);
			this.form = this.$rootScope.form;
